...
DOC_EMB_PT   = "/home/alpaco/autosry/cde_doc_emb.pt"     # (N, D) float32
DOCS_NPY     = "/home/alpaco/autosry/cde_docs.npy"       # (N,) list[str]
QUERY_EMB_PT = "/home/alpaco/autosry/cde_query_emb.pt"   # (선택)

# CDE / 생성 모델
CDE_NAME = "jxm/cde-small-v2"
GEN_NAME = "skt/A.X-4.0-Light"     # 필요 시 "SKT-AI/LLM-AX-4.0-Light"
...
# =========================
doc_embs = torch.load(DOC_EMB_PT, map_location="cpu")  # (N, D) float32
docs     = np.load(DOCS_NPY, allow_pickle=True).tolist()

index = faiss.IndexFlatIP(doc_embs.shape[1])  # IP = cosine (L2 normalized)
...
def ensure_cde_and_context():
    ...
    if len(docs) >= minicorpus_size:
        minicorpus_docs = docs[:minicorpus_size]  # 재현성 목적(원하면 random.sample로 변경)
    else:
    ...
    with torch.no_grad():
        for i in tqdm(range(0, mc_tok["input_ids"].size(0), BATCH_SIZE), desc="CDE 1st-stage (context build)"):
            ...
            v = model_cde.first_stage_model(**batch)  # (B, H)
            vecs.append(v)
...
def search_dense_by_text(query_text: str, topk=5):
    ...
    q = embed_query_cde(query_text)  # (1, D)
...
def generate_section_text(section, project_name, depart_name, project_no, period, budget, keywords):
    ...
    out = gen_pipe(prompt, max_new_tokens=1000, do_sample=False)
...
def render_doc(project_name, depart_name, project_no, period, budget, keywords):
    ...
    for section in sections:
        ...
        text = generate_section_text(section, project_name, depart_name, project_no, period, budget, keywords)
        doc.add_paragraph(text)

    outpath = "RND_Plan.docx"
    doc.save(outpath)
    print(f"[DONE] {outpath} 생성 완료")
